...
// -----------------------------------------------------------------
    ...
    /**
     * Es el c�digo de error para cuando ya hay un carro en el parqueadero con la placa de un nuevo
     * carro que va a entrar.
     */
     ...
     * @return Mensaje con la placa. Si no hay un carro en dicha posici�n retorna un mensaje indicando
     * que no hay un carro en esa posici�n.
     */
     ...
     * @return Retorna el valor a pagar. Si el carro no se encontraba dentro del parqueadero entonces
     * retorna CARRO_NO_EXISTE. <br>
    ...
    /**
     * Indica el n�mero de puesto en el que se encuentra el carro con una placa dada. Si no lo
     * encuentra retorna el valor CARRO_NO_EXISTE.
    ...
    /**
     * Indica el tiempo promedio de los carros del parqueadero.
     * @return Retorna el tiempo promedio de los carros
     */
    public double darTiempoPromedio(){
        ...
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                totalCarros++;
                Carro carrito= puesto.darCarro();
                totalTiempo+=carrito.darTiempoEnParqueadero(horaActual);
            }
        }
        if(totalCarros==0){
            return 0;
        }
        double tiempoPromedio=(double)totalTiempo/totalCarros;
        return tiempoPromedio;
    }

    /**
     * Indica el verdadero si un carro tiene más de 8 horas y false si está menos de 8 horas
     * @return Retorna TRUE o FALSE según se haya o no excedido el carro
     */
    public boolean hayCarroMasDeOchoHoras() {
        boolean carrosExcedidos8Horas= false;
        for (Puesto puesto: puestos){
            if (puesto.estaOcupado()){
                Carro carros= puesto.darCarro();
                int tiempo= carros.darTiempoEnParqueadero(horaActual);
                if (tiempo>8){
                    carrosExcedidos8Horas=true;
                }
            }
        }
        return carrosExcedidos8Horas;
    }

    /**
     * Indica la Lista de carros con tiempo mayor a 3 horas
     * @return Retorna un Lista de Carros parqueados más de 3 horas
     */
    public ArrayList<Carro> darCarrosMasDeTresHorasParqueados( ) {
        ArrayList<Carro> carros3Horas= new ArrayList<>();
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                Carro carros= puesto.darCarro();
                int tiempo= carros.darTiempoEnParqueadero(horaActual);
                if (tiempo>3){
                    carros3Horas.add(carros);
                }
            }
        }
        return carros3Horas;
    ...
    /**
     * Indica si hay carros con la misma placa
     * @return Retorna TRUE si es que hay carros con la misma placa y FALSE en el caso contrario
     */
    public boolean hayCarrosPlacaIgual() {
        for (int i = 0; i < TAMANO; i++) {
            if (puestos[i].estaOcupado()) {
                Carro carroI = puestos[i].darCarro();
                String placaI = carroI.darPlaca();

                for (int j = i + 1; j < TAMANO; j++) {
                    if (puestos[j].estaOcupado()) {
                        Carro carroJ = puestos[j].darCarro();
                        String placaJ = carroJ.darPlaca();

                        if (placaI.equals(placaJ)) {
                            return true;
                        }
                    }
                }
    ...
    /**
     * Indica si hay carros con placa que comience en PB
     * @return Retorna la cantidad de carros que cumplan ese requisito
     */

    public int carrosPlacaPB (){
        int contador=0;
        for(Puesto puesto: puestos){
            if (puesto.estaOcupado()){
                Carro carros =puesto.darCarro();
                String placaPB= carros.darPlaca().substring(0,2);
                if (placaPB.equals("PB")){
                    contador++;
                }
            }
        }
    ...
    /**
     * Indica si hay carros con 24 horas o más en el parqueadero
     * @return Retorna TRUE si es que hay carros con 24 horas o más y FALSE en el caso contrario
     */
    public boolean carroMas24Horas(){
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                Carro carros= puesto.darCarro();
                int tiempo = carros.darTiempoEnParqueadero(horaActual);
                if (tiempo>=24){
                    System.out.println("Este carro ha estado parqueado 24 horas o mas");
                    return true;
                }
            }
        }
    ...
    /**
     * Desocupa todos los carros del parqueadero
     * @return Retorna la cantidad de carros que habían en el parqueadero
     */
     ...
     * @return Retorna uno de los siguientes mensajes:
     * Cantidad de carros con placa PB: X – Hay carro parqueado por 24 o más horas: Sí.
     * Cantidad de carros con placa PB: X – Hay carro parqueado por 24 o más horas: No.
     */
    public String metodo1( )
    {
        int cantidadCarrosPB = carrosPlacaPB();
        boolean carro24Horas = carroMas24Horas();
     ...
     * @return Retorna en forma de mensaje la cantidad de carros sacados
     */
    public String metodo2( )
    {
        int cantidadCarrosSacados= desocuparParqueadero();
        return "La cantidad de carros sacados fue: "+cantidadCarrosSacados;
    }
